...
Each stream can only have a single `callback`. If you try to `observe()` the same stream twice, a warning will be emitted and the second `callback` will never receive any data. In other words, the stream will always look like an empty stream (except to the first observer). This way, observers either get "all or nothing" — it's impossible to receive a partial representation of the stream's content.
