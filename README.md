...
Streams cannot be fulfilled until all of their data has been consumed. Sometimes the data is consumed by a new stream (such as in [`.map()`](#mapconcurrency-callback---stream)). Other times it is consumed by a process that results in single value ([`.merge()`](#merge---this), [`.reduce()`](#reducecallback-initialvalue---this)).
