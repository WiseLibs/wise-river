...
Each stream can only have a single `callback`. If you try to `observe()` the same stream twice, it will be a no-op and a warning will be emitted.
...
Constructs an array containing all the data written to the stream, and returns a promise for that array. The items in the array will appear in the order that the stream received them.
