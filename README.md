...
Each stream can only have a single `callback`. If you try to `observe()` the same stream twice, it will be a no-op and a warning will be emitted. In other words, observers beyond the first will always see the stream as an empty stream. This way, observers either get "all or nothing" — it's impossible to receive a partial representation of the stream's content.
