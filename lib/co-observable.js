...
const cancelCoObservable = (obsv) => () => {
	if (obsv[shared.onabort] !== noop) {
		obsv[shared.onabort](new Cancellation('An upstream consumer cancelled the river chain'));
	}
};

const rethrow = (() => {
	const g = new Function('return this')();
	const schedule = g != null && g.process != null && typeof g.process.nextTick === 'function' ? g.process.nextTick : fn => setTimeout(fn, 0);
