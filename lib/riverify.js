...
WiseRiver.riverify = (streamlike, { decouple = false } = {}) => {
	...
	if (isAsyncIterable(streamlike)) {
		return new WiseRiver((resolve, reject, write, free) => {
			...
			const useCleanup = !decouple && typeof iterator.return === 'function';
			const fail = useCleanup ? (reason) => { done = true; reject(reason); } : reject;
			Promise.resolve(iterator.next()).then(function item(record) {
				try {
					if (record.done) { done = true; return resolve(); }
					write(record.value);
					Promise.resolve(iterator.next()).then(item, fail);
				} catch (reason) {
					fail(reason);
				}
			}, fail);
