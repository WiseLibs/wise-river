...
class WiseRiver extends BaseClass {
	...
	static riverify(streamlike, { decouple = false } = {}) {
		...
		if (isAsyncIterable(streamlike)) {
			return new WiseRiver((resolve, reject, write, free) => {
				...
				iterator.next().then(function item({ value, done }) {
					if (done) return resolve();
					write(value);
					iterator.next().then(item, reject);
				}, reject);
				if (!decouple && typeof iterator.return === 'function') {
					free(() => { iterator.return(); });
				}
			});
		}
	}
...
const isAsyncIterable = x => typeof Symbol.asyncIterator === 'symbol' && typeof x[Symbol.asyncIterator] === 'function';
