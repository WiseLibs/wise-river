...
class WiseRiver extends BaseClass {
	...
	static from(iterable) {
		return new WiseRiver((resolve, reject, write) => {
			if (notIterable(iterable)) return reject(new TypeError('Expected argument to be an iterable object'));
	...
	fork(count = 2) {
		if (!validCount(count)) throw new TypeError('Expected count to be an integer between 1 and 4294967295');
		const forks = new Array(count);
		const inners = new Array(count);
		const cancel = cancelForForks(count, pumpForForks(this, inners));
		const assignInner = (resolve, reject, write, free) => {
			inners[i] = { resolve, reject, write };
			free(cancel);
		};
		let i = 0;
		for (; i < count; ++i) forks[i] = new WiseRiver(assignInner);
		return forks;
	}
	...
	until(promise) {
		return new WiseRiver((resolve, reject, write, free) => {
			...
			free(cancel);
			Promise.resolve(promise).then(() => { resolve(); cancel(); }, reject);
			this.then(resolve, reject);
		});
	}
	...
	includes(value, equals = strictEquals) {
		return new Promise((resolve, reject) => {
			...
			if (!Promise.isPromise(value)) return search(value);
			Promise.resolve(value).then(search, (reason) => {
				this.pump(noop)();
				reject(reason);
			});
		});
	...
	first(count) {
		return new Promise((resolve, reject) => {
			if (arguments.length === 0) {
				...
				reject(new TypeError('Expected count to be an integer between 1 and 4294967295'));
			}
		});
	}
...
const validCount = x => Number.isInteger(x) && x >= 1 && x <= 0xffffffff;
