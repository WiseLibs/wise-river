...
class WiseRiver extends BaseClass {
	...
	static combine(...args) {
		return new WiseRiver((resolve, reject, write, free) => {
			...
			for (const river of rivers) {
				if (river instanceof WiseRiver) free(river.pump(write));
			}
	...
	fork(count = 2) {
		...
		const cleanup = cleanupForForks(forks.length, pumpForForks(this, inners));
	...
	map(concurrency, handler) {
		return new WiseRiver((resolve, reject, write, free) => {
			...
			free(this.pump(concurrency, upgrade(handler, (item) => {
				...
				if (mapped instanceof WiseRiver) { free(mapped.pump(write)); return mapped; }
	...
	while(concurrency, handler) {
		return new WiseRiver((resolve, reject, write, free) => {
			if (typeof concurrency === 'function') { handler = arguments[0]; concurrency = arguments[1]; }
			const cleanup = this.pump(concurrency, upgrade(handler, (item) => {
				const bool = handler(item);
				if (!Promise.isPromise(bool)) {
					if (bool) write(item);
					else { resolve(); cleanup(); }
					return;
				}
	...
	until(promise) {
		return new WiseRiver((resolve, reject, write, free) => {
			const cleanup = this.pump(write);
			const finish = () => { resolve(); cleanup(); };
			free(cleanup);
	...
	consume(concurrency, handler) {
		return new Promise((resolve, reject) => {
			const cleanup = this.pump(concurrency, handler);
	...
	reduce(handler, result) {
		return new Promise((resolve, reject) => {
			...
			const cleanup = this.pump(1, upgrade(handler, (item) => {
				if (firstItem) {
					firstItem = false;
	...
	all() {
		return new Promise((resolve, reject) => {
			const result = [];
			const cleanup = this.pump((item) => { result.push(item); });
			this.then(() => { resolve(result); cleanup(); },
				(reason) => { reject(reason); cleanup(); });
		});
	}
