...
/*
	Observable is like a WiseRiverBase, except:
...
class Observable extends FastQueue {
	...
	[shared.attachHandler](handlerFunction, concurrencyLimit = 0) {
		if (this[handler] !== unassigned) {
			if (this[handler] === noop) shared.warn('This river was already resolved', Observable.prototype[shared.attachHandler]);
			else shared.warn('This river already has a destination (use .fork() instead)', Observable.prototype[shared.attachHandler]);
...
const cancelObservable = (obsv) => () => {
	if (obsv[shared.onabort] !== noop) {
		obsv[shared.onabort](new Cancellation('An upstream observer cleaned up the event source'));
	}
};
