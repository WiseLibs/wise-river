...
class HonestStream extends BaseClass {
	...
	reduce(handler, result) {
		return new Promise((resolve, reject) => {
			...
			let firstItem = true;
			const calc = (a, b, i) => {
				const ret = handler(a, b, i);
				if (!Promise.isPromise(ret)) { result = ret; return; }
				return Promise.resolve(ret).then((ret) => { result = ret; });
			};
			this.observe(1, upgrade(handler, (item, i) => {
				if (firstItem) {
					firstItem = false;
					if (arguments.length < 2) { result = item; return; }
					if (!(result instanceof Promise)) return calc(result, item, i);
					return result.then(seed => calc(seed, item, i));
				}
				return calc(result, item, i);
			}));
