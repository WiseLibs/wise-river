...
class HonestStream extends BaseClass {
	...
	static every(ms) {
		return new HonestStream((resolve, reject, write, free) => {
	...
	static combine(...args) {
		return new HonestStream((resolve, reject, write, free) => {
			...
			for (const stream of streams) {
				if (stream instanceof HonestStream) free(stream.observe(write));
			}
	...
	fork(count = 2) {
		...
		let i = 0;
		for (let len = forks.length; i < len; ++i) forks[i] = new HonestStream(assignInner);
		return forks;
	}
	...
	map(concurrency, handler) {
		return new HonestStream((resolve, reject, write, free) => {
			...
			free(this.observe(concurrency, upgrade(handler, (item) => {
				...
				if (mapped instanceof HonestStream) { free(mapped.observe(write)); return mapped; }
